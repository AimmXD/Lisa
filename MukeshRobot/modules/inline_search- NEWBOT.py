...
from MukeshRobot import BOT_USERNAME, app, user
from MukeshRobot.helper import GENRES_EMOJI, fetch, post_to_telegraph, search_jw
from MukeshRobot.plugins.dev import shell_exec
from MukeshRobot.plugins.misc_tools import calc_btn
from MukeshRobot.vars import USER_SESSION
...
async def inline_menu(self, inline_query: InlineQuery):
    if inline_query.query.strip().lower().strip() == "":
        ...
        msg = f"""
**[MukeshRobot✨](https://github.com/yasirarism):**
...
"""
        ...
        answerss = [
            ...
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    f"<b>Github Repo @{BOT_USERNAME}</b>\n\nhttps://github.com/yasirarism/MukeshRobotPyro"
                ),
...
async def imdb_inl(_, query):
    ...
    if cbuser == f"{query.from_user.id}":
        try:
            ...
            res_str += "<b>©️ IMDb by</b> @MukeshRobotRoBot"
